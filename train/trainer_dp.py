...
class Trainer():
    def __init__(self,args):
        ...
        if args.isval:
            ...
            self.val_loader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size=2)
    ...
    def train(self,deblur_model,train_dataloader,optim,scheduler,train_writer,start_epoch,all_step):
        ...
        for epoch in range(start_epoch,self.max_epoch+1):
            ...
            for iteration, data in enumerate(train_dataloader):
                ...
                loss_l1 = sum([self.l1Loss(output_module[j],gt_module[j]).cpu().detach().numpy() for j in range(len(output_module))])
                loss_fft = sum([self.fftLoss(output_module[j],gt_module[j]).cpu().detach().numpy() for j in range(len(output_module))])
